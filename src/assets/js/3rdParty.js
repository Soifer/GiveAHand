...
var NanaTaboola = {
  ...
  insertHeader: function () {
    console.log('TABOOLA >>>>>>> insertHeader: ' + this.objectType," <<<<<<<<");
  ...
  insertFooter: function () {
    console.log('TABOOLA >>>>>>> insertFooter : ' + this.objectType," <<<<<<<<");
    window._taboola = window._taboola || [];
    _taboola.push({
      flush: true
    });
  ...
  initContent: function (id, placement, mode, type) {
    window._taboola = window._taboola || [];
    console.log("TABOOLA >>>>>>> init (initContent) <<<<<<<<", mode);
  ...
  reloadContent: function (id, placement, mode, type, currentUrl) {
    window._taboola = window._taboola || [];
    _taboola.push({
      notify: 'newPageLoad',
      mode: mode,
     ...
     console.log('TABOOLA >>>>>>> reloadContent (newPageLoad) with 5 params: ', id, placement, mode, type, currentUrl, "<<<<<<<<");
  }
};
  ...
  this.initGeneral = function () {
    googletag.cmd.push(function () {
      for (var i = 0; i < AdUnitsCollectionIndex.list.length; i++) {
        ...
        // For dev targeting only
        if (DEV_ENV) googletag.pubads().setTargeting('10tv', 'dev'); // REMOVE ON PRODUCTION
...
var castTimeHelper = {
  ...
  routeHandler: function (data) {
    castTimeHelper.platform = castTimeHelper.getMobileOperatingSystem();
    ...
    if (castTimeHelper.platform === "android" && typeof Android !== "undefined") {
      ...
      console.log('routeHandler',castTimeHelper.routerHelper);
    }
  },
